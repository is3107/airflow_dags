...
def test_dag_file_names_matches_pattern():
    logging.info(f"Checking that DAG ID follows either the ingestion pattern: {ingestion_pattern}.py or the analytics pattern: {analytics_pattern}.py")
    wrong_names = []
    ...
    for file_name in filenames:
        if not re.fullmatch(ingestion_pattern + '.py', file_name) and not re.fullmatch(analytics_pattern + '.py', file_name):
            ...
            logging.info(f"{file_name} matched successfully")

    for names in wrong_names:
        logging.warning(f"The DAG with file name: {names} does not match the name formats")

    assert len(wrong_names) == 0

def test_dag_id_matches_pattern(dag_bag):
    wrong_names = []
    logging.info(f"Checking that DAG ID follows either the ingestion pattern: {ingestion_pattern} or the analytics pattern: {analytics_pattern}")
    for dag_id, dag in dag_bag.dags.items():
        if not re.fullmatch(ingestion_pattern, dag_id) and not re.fullmatch(analytics_pattern, dag_id):
            ...
            logging.info(f"{dag_id} matched successfully")
